...
class CustomEnv:
    ...
    def __init__(self, df, initial_balance=1000, lookback_window_size=50, Render_range=100, Show_reward=True, Show_indicators=True, normalize_value=40000):
    ...
    def step(self, action):
        ...
        if action == 0: # Hold
            ...
            sumBuy = self.crypto_bought * current_price
            total_Buy = self.crypto_bought
            current_buy = current_price
            with open("log_Buy.txt", "a+") as results:
                current_date = datetime.now().strftime('%Y-%m-%d %H:%M')
                results.write(f'{current_date}')
                results.write(f', High:{High}, Low:{Low}, total:{self.crypto_bought},current_price:{current_price},fee:{(0.1*self.crypto_bought*current_price)}\n')
            ...
            sumSell = self.crypto_sold * current_price
            total_Sell = self.crypto_sold
            current_Sell = current_price
            self.trades.append({'Date' : Date, 'High' : High, 'Low' : Low, 'total': self.crypto_sold, 'type': "sell", 'current_price': current_price})
            self.episode_orders += 1
            
            with open("log_Sell.txt", "a+") as results:
                current_date = datetime.now().strftime('%Y-%m-%d %H:%M')
                results.write(f'{current_date}')
                results.write(f', High:{High}, Low:{Low}, total:{self.crypto_sold},current_price:{current_price},fee:{(0.1*self.crypto_sold*current_price)}\n')
                
        # with open("logPL.txt", "a+") as results:
        #     current_date = datetime.now().strftime('%Y-%m-%d %H:%M')
        #     results.write(f'{current_date}')
        #     results.write(f', High:{High}, Low:{Low}, total-Buy:{total_Buy}, current_price:{current_buy}, total-sell:{total_Sell},current_price:{current_Sell}, P/L: {sumSell - 1000} \n')
...
def test_agent(env, agent, visualize=False, test_episodes=10, folder="", name="Crypto_trader", comment=""):
    ...
    for episode in range(test_episodes):
        ...
        while True:
            ...
            if env.current_step == env.end_step:
                ...
                average_orders += env.episode_orders
                if env.net_worth < env.initial_balance: no_profit_episodes += 1 # calculate episode count where we had negative profit through episode
                print("episode: {:<5}, net_worth: {:<7.2f}, average_net_worth: {:<7.2f}, orders: {}, action: {}".format(episode, env.net_worth, average_net_worth/(episode+1), env.episode_orders,action))
                with open("logPL20-21June.txt", "a+") as results:
                    current_date = datetime.now().strftime('%Y-%m-%d %H:%M')
                    results.write(f'{current_date}')
                    results.write(f', net_worth:{env.net_worth}, P/L: {env.net_worth - 1000} \n')
                break
...
if __name__ == "__main__":            
    ...
    # df = yfinance.download("BTC-USD", start="2015-01-01", end="2021-08-30")
    # df = df.reset_index()   
    ...
    # df = df['2020-06-20' :'2021-07-24']
    # df = df.reset_index()
    # df['Date'] = pd.to_datetime(df['Date'], format='%Y-%m-%d %H:%M:%S').dt.strftime('%d/%m/%Y %H:%M')
    ...
    test_agent(test_env, agent,visualize=False, test_episodes=1000, folder="2021_09_29_17_45_Crypto_trader", name="20645905.37_Crypto_trader", comment="20-21June 30day")
